...
public class ColumnPositionMappingBeanListHandlerTest {
	...
	public void testLoad1() throws IOException {
		...
		try (CsvReader reader = new CsvReader(new StringReader("symbol,name,price,volume\r\nAAAA,aaa,10000,10\r\nBBBB,bbb,NULL,0"), cfg)) {
			final List<SampleBean> list = new ColumnPositionMappingBeanListHandler<>(SampleBean.class).load(reader);
	...
	public void testLoad2() throws IOException {
		...
		try (CsvReader reader = new CsvReader(new StringReader("symbol,name,price,volume\r\nAAAA,aaa,10000,10\r\nBBBB,bbb,NULL,0"), cfg)) {
			final List<SampleBean> list = new ColumnPositionMappingBeanListHandler<>(SampleBean.class)
					.addColumn("symbol")
					.addColumn("name")
					.addColumn("price")
					.addColumn("volume")
	...
	public void testLoad6() throws IOException {
		cfg.setSkipLines(1);	// 項目位置指定時はヘッダは不要なので読飛ばす指定をする
		try (CsvReader reader = new CsvReader(new StringReader("symbol,name,price,volume\r\nAAAA,aaa,10000,10\r\nBBBB,bbb,NULL,0"), cfg)) {
			final Map<Integer, String> columnMapping = new HashMap<>();
			columnMapping.put(3, "volume");
			columnMapping.put(2, "price");
			columnMapping.put(0, "symbol");
			columnMapping.put(1, "name");

			final List<SampleBean> list = new ColumnPositionMappingBeanListHandler<>(SampleBean.class).columnMapping(columnMapping).load(reader);
	...
	public void testLoad8() throws IOException {
		cfg.setSkipLines(0);
		try (CsvReader reader = new CsvReader(new StringReader("AAAA,aaa,10\\,000,10,2008/10/28,10:24:00\r\nBBBB,bbb,NULL,0,NULL,NULL"), cfg)) {
			final List<SampleBean> list = new ColumnPositionMappingBeanListHandler<>(SampleBean.class)
					.addColumn(0, "symbol")
					.addColumn(1, "name")
...
//				"symbol,name,price,volume,date\r\n" +
	...
	public void testSaveNoHeader() throws IOException {
		...
		try (CsvWriter writer = new CsvWriter(sw, cfg)) {
			new ColumnPositionMappingBeanListHandler<>(SampleBean.class).header(false).save(list, writer);
		}
	...
	public void testSave1() throws IOException {
		...
		final StringWriter sw = new StringWriter();
		try (CsvWriter writer = new CsvWriter(sw, cfg)) {
			new ColumnPositionMappingBeanListHandler<>(SampleBean.class).save(list, writer);
		}
	...
	public void testSave2() throws IOException {
		...
		final StringWriter sw = new StringWriter();
		try (CsvWriter writer = new CsvWriter(sw, cfg)) {
			new ColumnPositionMappingBeanListHandler<>(SampleBean.class)
					.addColumn("name")
					.addColumn("symbol")
					.addColumn("price")
					.addColumn("volume")
