...
public class CsvEntityListHandlerTest {
	...
	public void testCsvEntityListHandlerIllegalArgumentException2() {
		new CsvEntityListHandler<>(SampleBean.class);
	}
	...
	public void testLoadPrice() throws IOException {
		try (CsvReader reader = new CsvReader(new StringReader("シンボル,名称,価格,出来高,日付,時刻\r\nAAAA,aaa,10\\,000,10,2009/10/28,10:24:00\r\nBBBB,bbb,NULL,0,NULL,NULL"), cfg)) {
			final List<Price> list = new CsvEntityListHandler<>(Price.class)
					.load(reader);
	...
	public void testLoadPrice2() throws IOException {
		try (CsvReader reader = new CsvReader(new StringReader("シンボル,名称,価格,出来高,日付,時刻\r\nAAAA,aaa,10\\,000,10,2009/10/28,10:24:00\r\nBBBB,bbb,NULL,0,NULL,NULL"), cfg)) {
			final List<Price2> list = new CsvEntityListHandler<>(Price2.class)
					.load(reader);
	...
	public void testLoadOffsetLimit() throws IOException {
		try (CsvReader reader = new CsvReader(new StringReader("シンボル,名称,価格,出来高,日付,時刻\r\nAAAA,aaa,10\\,000,10,2009/10/28,10:24:00\r\nBBBB,bbb,NULL,0,NULL,NULL"), cfg)) {
			final List<Price> list = new CsvEntityListHandler<>(Price.class).offset(1).limit(1).load(reader);
	...
	public void testSave() throws Exception {
		...
		try (CsvWriter writer = new CsvWriter(sw, cfg)) {
			new CsvEntityListHandler<>(Price.class).save(list, writer);
		}
	...
	public void testSaveFilter() throws Exception {
		...
		final StringWriter sw = new StringWriter();
		try (CsvWriter writer = new CsvWriter(sw, cfg)) {
			new CsvEntityListHandler<>(Price.class)
					.filter(new SimpleCsvNamedValueFilter().ne("シンボル", "gcu09", true))
					.filter(new SimpleBeanFilter().ne("date", df.parse("2008/11/06 12:00:00")))
					.save(list, writer);
		}
